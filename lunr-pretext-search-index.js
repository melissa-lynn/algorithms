...
},
{
  ...
  "body": " Selection Sort  Selection sort works by repeatedly finding the next smallest item in the array, and moving it into the correct position. Here is pseudocode for selection sort.  selection_sort input: array of integers of length n state change: array of integers sorted into ascending order for each position in the array: find the smallest item in the array, at or after the current position swap the item at the current position with the item just found return  Here is more detailed pseudocode, that we will use for our runtime analysis. This pseudocode also makes it straightforward to implement the algorithm in many programming languages.  selection_sort input: array of integers of length n state change: array of integers sorted into ascending order for i from 0 to n: min_index gets 0 for j from i to n: if array[j] \\lt array[min_index]: min_index gets j swap array[i] and array[min_index] return  "
},
{
  ...
  "body": " Insertion Sort  Insertion sort works by sorting the array from the beginning, adding one item at a time by finding its correct position in the sorted portion of the array.  insertion_sort input: array of integers of length n state change: array of integers sorted into ascending order for each position in the array: current is the item at the current position while current is less than the item in the previous position: swap current with that item return  Here is more detailed pseudocode.  insertion_sort input: array of integers of length n state change: array of integers sorted into ascending order for i from 1 to n: j gets i-1 while array[i] \\lt array[j] and j \\geq 0: swap array[i] and array[j] i gets i-1 j gets j-1 return  "
},
{
  ...
  "number": "3.3",
  "title": "Bubble Sort",
  "body": " Bubble Sort  Bubble sort works by iterating through the array, swapping pairs of items that are out of order. It repeats this iteration until the entire array is in order.  bubble_sort input: array of integers of length n state change: array of integers sorted into ascending order while there was a swap in the previous iteration: for each position in the array: if the next item is less than the current item: swap the current item with the next item return  Here is more detailed pseudocode.  bubble_sort input: array of integers of length n state change: array of integers sorted into ascending order num_swaps gets 1 while num_swaps \\gt 0: num_swaps gets 0 for i from 0 to n-1: if array[i+1] \\lt array[i]: swap array[i] and array[i+1] num_swaps += 1 return  Here is another version of bubble sort.  bubble_sort input: array of integers of length n state change: array of integers sorted into ascending order for r from 1 to n: num_swaps gets 0 for i from 0 to n-r: if array[i+1] \\lt array[i]: swap array[i] and array[i+1] num_swaps += 1 if num_swaps = 0: break return  "
},
{
  ...
  "number": "3.4",
  "title": "Merge Sort",
  "body": " Merge Sort  Merge sort works by dividing the array into its first half and second half, sorting each half, then merging the two halves resulting in a sorted array.  merge_sort input: array of integers of length n state change: array of integers sorted into ascending order if the length of the array is 0 or 1: return call merge_sort to sort the first half of array call merge_sort to sort the second half of the array new_array is initialized as an empty array iterate through the first half and second half of the array simultaneously if the current item in the first half is less than the current item in the second half: append the current item from the first half to new_array go to the next item in the first half else: append the current item from the second half to new_array go to the next item in the second half if there are remaining items in the first half of the array: append them to new_array if there are remaining items in the second half of the array: append them to new_array copy new_array into the original array return  Here is more detailed pseudocode for merge sort.  merge_sort input: array of integers of length n state change: array of integers sorted into ascending order call merge_sort_helper on array, 0, n-1 return merge_sort_helper input:array of integers of length n, start_index, stop_index state change: sorts the integers from start_index through stop_index into ascending order if start_index \\geq stop_index: return call merge_sort_helper on array, start_index, (start_index+stop_index)\/\/2 call merge_sort_helper on array, (start_index+stop_index)\/\/2 + 1, stop_index new_array is initialized as an empty array i gets start_index j gets (start_index+stop_index)\/\/2 + 1 k gets 0 while i \\leq (start_index+stop_index)\/\/2 and j \\leq stop_index: if array[i] \\leq array[j]: new_array[k] gets array[i] i gets i+1 k gets k+1 else: new_array[k] gets array[j] j gets j+1 k gets k+1 while i \\leq (start_index+stop_index)\/\/2: new_array[k] gets array[i] i gets i+1 k gets k+1 while j \\leq stop_index: new_array[k] gets array[j] j gets j+1 k gets k+1 for k from 0 through stop_index-start_index+1: array[start_index+k] gets new_array[k] return  "
},
{
  ...
  "number": "3.5",
  "title": "Quick Sort",
  "body": " Quick Sort  Quick Sort works by choosing an item from the array, called the pivot. The array is reordered so that every item before the pivot is less than the pivot, and every item after the pivot is greater than the pivot. Then, the portion of the array before the pivot is sorted, and the portion of the array after the pivot is sorted, resulting in a sorted array.  quick_sort input: array of integers of length n state change: array of integers sorted into ascending order the pivot is the last item in the array for each position in the array: if the current item is less than the pivot: swap it with the first item that is greater than the pivot swap the pivot with the first item that is greater than the pivot call quick_sort on the portion of the array before the pivot call quick_sort on the portion of the array after the pivot return  Here is more detailed pseudocode for quick sort.  quick_sort input: array of integers of length n state change: array of integers sorted into ascending order call quick_sort_helper on array, 0, n-1 quick_sort_helper input: array of integers of length n, start_index, stop_index state change: array of integers sorted into ascending order from start_index through stop_index if stop_index \\leq start_index: return pivot gets array[stop_index] j gets 0 for i from start_index to stop_index: if array[i] \\lt pivot: swap array[j] and array[i] j gets j+1 swap array[j] and array[stop_index] call quick_sort on the portion of the array before the pivot call quick_sort on the portion of the array after the pivot return  "
},
{
  ...
  "number": "3.6",
  "title": "Tree Sort",
  "body": " Tree Sort  Tree sort works by inserting all items in a binary search tree, then performing an in-order traversal of the tree to extract the items in increasing order.  tree_sort input: array of integers of length n state change: array of integers sorted into ascending order tree is an empty binary search tree for each item in the array: insert the item in the tree traversing the tree in order, copy the items back into the array return  Here is more detailed pseudocode for tree sort.  tree_sort input: array of integers of length n state change: array of integers sorted into ascending order tree is an empty binary search tree for each item in the array: insert the item in the tree call tree_sort_helper on the root of the tree, array, 0 return tree_sort_helper(tree_node, array, index) input: node in a tree, array, index state_change: all descendents of the node are copied into array in ascending order starting at index, index is updated to the end of the copied portion if tree_node is empty: return call tree_sort_helper on the left child of tree_node, array, index array[index] gets the data at tree_node index gets index + 1 call tree_sort_helper on the right child of tree_node, array, index return  "
},
{
